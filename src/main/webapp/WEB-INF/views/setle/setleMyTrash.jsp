		...
		<div class="container flex">
			...
			<div class="content">
				<div class="txt-sear-box flex ju-sp-bt al-it-ce">
					<h1>
						<a href="<%=request.getContextPath()%>/setle/setleMyList">휴지통</a> <span>워크플로우 <i class="fa-solid fa-angle-right"></i> 휴지통
						</span>
					</h1>
					...
					<div class="add-sear" style="display: inline-block;">
                        <ul class="flex">
                                <li id="restoreA"><i class="fa-solid fa-arrow-rotate-left"></i>복구</li>
								...
								<c:forEach var="setle" items="${setleList}" varStatus="status">
									<tr onclick='location.href="<%=request.getContextPath()%>/setle/setleDetail?setleNo=${setle.setleNo }"'>
										...
										<td>
											...
											<fmt:formatDate value="${setle.writngOn }"
													pattern="yyyy.MM.dd" /><br />
											<%-- <fmt:formatDate value="${setle.writngOn }" pattern="HH:mm:ss" /> --%>
											
												<div class="deadlineTime"><span>D-3일</span></div>
											</div>
										</td>
										
										<td class="o-title"><div>${setle.writerDepart }</div></td>
										<td><c:choose>
...
const restoreACodeUpdate = document.querySelector('#restoreA');
const removeCodeUpdate = document.querySelector('#removeB');
let isArrayForAtch = [];

restoreACodeUpdate.addEventListener('click', function(e){
	...
	if(isArrayForAtch.length == 0){
		alert("이동할 결재문서를 선택하세요");
	}
	else{
		$.ajax({
			url : '<%=request.getContextPath()%>/setle/sendMyList',
			type : 'post',
...
removeCodeUpdate.addEventListener('click', function(e){
	...
	console.log(isArrayForAtch);
	if(isArrayForAtch.length == 0){
		alert("영구삭제할 결재문서를 선택하세요");
	}
	else{
		$.ajax({
			url : '<%=request.getContextPath()%>/setle/removeSetle',
			type : 'post',
			data : {setleNoList : isArrayForAtch},
			success : function(result){
				if(result == 'success'){
