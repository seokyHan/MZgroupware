...
<body>
	<div class="wrap">
		...
		<c:set var="emp" value="${dataMap.emp }" />
		...
		<div class="container flex">
			...
			<div class="sub-content">
				<div class="backPage"
					onClick="location.href='<%=request.getContextPath()%>/customBoard/${cboard.boardtype_no}'">
					...
					<div class="de-con con01">
						...
						<div class="detail-title">
							<div class="txt flex ju-sp-bt al-it-ce">
                                 <h2>${cboard.board_title }</h2> 
                                 <input type="hidden" id="unity" value="${cboard.unityatchmnflno }"/>
                                <c:if test="${loginEmp.empl_no eq cboard.board_writer }">
	                                <div class="de-btn">
	                                    <button class="modi" >수정</button>
	                                    <button class="remo">삭제</button>
	                                </div>                            
                                </c:if>
                                <c:if test="${loginEmp.empl_no ne cboard.board_writer }">
							...
							<div class="user flex ju-sp-bt al-it-ce">
								<div class="flex">
									...
									<div class="user-name">
										<p>${cboard.ncnm }</p>
										<span><i class="fa-solid fa-briefcase"></i>${emp.dept }</span>
									</div>
								</div>

								<div class="free-like" id="board_likeNum">

									<span id="board_like">${cboard.board_like }</span>
							...
							<tbody>
								<tr>
									...
									<td id="registDate"><fmt:formatDate
											value="${cboard.board_rgdt  }" pattern="yyyy-MM-dd" /></td>
								</tr>
								...
								<tr>
									<th>내용</th>
									<td colspan="3">${cboard.board_cont }</td>
								</tr>
							</tbody>
						</table>
					</div>
...
<form role="form">
	<input type="hidden" name="board_no" value="${cboard.board_no }"
		id="board_no" /> 
		<input type="hidden" name="boardtype_no"
		value="${cboard.boardtype_no }" id="boardtype_no" /> 
		<input type="hidden" name="empl_no" value="${cboard.boardtype_no }"
		id="boardtype_no" />
</form>
...
remo.addEventListener('click', function(){
	 console.log("dddddd");
	        	Swal.fire({
	                ...
	                if (result.isConfirmed) {
				        location.href='${pageContext.request.contextPath}/customBoard/remove?board_no='+${cboard.board_no}+'&boardtype_no='+${cboard.boardtype_no  };													                       
	                }
	            })	
	        })
	        modi.addEventListener('click',function(){	
		        ...
		        location.href='${pageContext.request.contextPath}/customBoard/modifyForm?board_no='+${cboard.board_no }+'&boardtype_no='+${cboard.boardtype_no };
	        })
...
// 		 alert("2"); 
 ...
 $.ajax({
              url : '<%=request.getContextPath()%>/customBoard/like',
			type : "post",
...
function callReplyList(page){
    ...
    $.ajax({
        ...
        data : {
        	board_no : ${cboard.board_no},
        	boardtype_no : ${cboard.boardtype_no},
        	page : page
        },            // Json 형식의 데이터이다.
...
function registReply(){
	...
	console.log('${cboard.board_no }');
	console.log('${cboard.boardtype_no }');
                                    ...
                                    <table>
                                        ...
                                        <tbody>
                                            <tr>
                                                ...
                                                <td><h3>{{ncnm}}<span>{{dept}}팀</span></h3> </td>
                                                <td class="right">
                                            ...
                                            <tr>
                                                <td><div class="reply-date">{{breply_regdate}}</div></td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </div>
