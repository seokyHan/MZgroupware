	...
	<%@ include file="/WEB-INF/include/topbar.jsp" %>
                ...
                <div class="p-wrap">
                    <div class="mail-send">
 				 	...
 				 	<form method="POST" enctype="multipart/form-data" id="registForm">
			             ...
			             <input type="hidden" name="mailSize" value="" id="mailSize"/>
                        ...
                        <div class="send-content">
                            <ul>
                                ...
                                <li class="flex ju-sp-bt">
                                    <label>보내는사람</label>
                                    <input type="text" value="${loginEmp.email }" readonly="readonly">
                                 ...
                                 <li class="flex">
                                    ...
                                    <div class="search-mailmem flex">
                                    	<div class="recentlyMail">
	                                    	<span class="addressMail">최근 사용한 주소</span>
	                                        <div class="in-user">
	                                        
	                                            <ul id="recentEmpSelect">
	                                            	<c:forEach items="${recentlyList }" var="rtl">
		                                                <li class="ckm flex al-it-ce">
		                                                    <input type="checkbox" name="chkReCent" value="${rtl.receEmp }" id="checkboxCK"> 
		                                                    <div>[${rtl.receNcnm}] ${rtl.receEmail}</div>
		                                                   	<input type="hidden" name="recentlyMail" value="${rtl.receEmail}" id="recentlyMail">
		                                                 </li>
	                                                </c:forEach>
	                                            </ul>
	                                        </div>
                                    	...
                                    	<div>
	                                    	<span class="addressMail">전체 주소록</span>
	                                        <div class="in-user">
	                                        
	                                            <ul id="sendEmpSelect">
	                                            	<c:forEach items="${empMailList }" var="empMail">
		                                                <li class="ckm flex al-it-ce">
		                                                    <input type="checkbox" name="checkMember" value="${empMail.empl_no }" id="checkboxCK"> 
		                                                    <div>[${empMail.ncnm}] ${empMail.email}</div>
		                                                   	<input type="hidden" name="receEmail" value="${empMail.email}" id="receEmail">
		                                                 </li>
	                                                </c:forEach>
	                                            </ul>
	                                        </div>
     ...
     <script>
       ...
       function rightMailEmp(){
	         ...
	         const recentEmpSelect = document.getElementById("recentEmpSelect").getElementsByTagName("li");
	         
	         const sendEmpList = document.getElementById("sendEmpList");
	         
	         const checkBox = document.getElementsByName("checkMember");
	         const chkReCent = document.getElementsByName("chkReCent");
	         
	         const mailPush = document.getElementsByName("receEmail");
	         const pushRecnt = document.getElementsByName("recentlyMail");
	         ...
	         for(var i = 0; i < recentEmpSelect.length; i++){
	            if(chkReCent[i].checked == true){
	            	
	            	console.log("체크확인 : "+chkReCent[i].value);
	            	console.log("체크메일확인 : "+mailPush[i].value);
	            	
	            	empList.push(chkReCent[i].value);
	            	mailList.push(pushRecnt[i].value);
	           
	            	sendEmpList.append(recentEmpSelect[i]);
       ...
       function leftMailEmp(){
    	   ...
    	   /* const reName = document.getElementsByName("receEmail");
    	   const rtName = document.getElementsByName("recentlyMail"); */
    	   
    	   const reName = document.getElementById("receEmail");
    	   const rtName = document.getElementById("recentlyMail");
    	  ...
    	  for(var i = 0; i < sendEmpList.length; i++){
    		 ...
    		 var testName1 = $(sendEmpList[i]).children("input#receEmail").attr('name');
    		 var testName2 = $(sendEmpList[i]).children("input#recentlyMail").attr('name');
    		 
    		 console.log("name1=" + testName1, "name2=" + testName2 );
    		  
   			if($(sendEmpList[i]).children("input").is(":checked") && testName1 == "receEmail"  &&  testName2 == undefined){
	   			   ...
	   			   for(let j = 0; j < empList.length; j++){
	                   if($(sendEmpList[j]).children("input").val() == $(sendEmpList[i]).children("input").val()){
	                	   ...
	                	   for(var t = 0; t > j; t++){
	   	            			...
	   	            			document.getElementById("sendEmpSelect").prepend(sendEmpList[t]);
	   	            		}
	                       break;
	                   }
	               }
   		  	 ...
   		  	 }else if($(sendEmpList[i]).children("input").is(":checked") && testName2 == "recentlyMail" &&  testName1 == undefined){
	                 ...
	                 if($(sendEmpList[j]).children("input").val() == $(sendEmpList[i]).children("input").val()){
   		  	  	...
   		  	  	document.getElementById("recentEmpSelect").prepend(sendEmpList[i]);
       ...
       function bytesToSize(bytes) {
		...
		function regist_go(){
			...
			Swal.fire({
				...
				document.querySelector("input[name=mailSize]").value=fileSize;
