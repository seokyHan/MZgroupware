...
<c:set var="pdsAttachList" value="${dataMapForM.pdsAttachList }" />
<c:set var="pdsFolderDeltail" value="${dataMapForM.pdsFolderDeltail }" />
<c:set var="pdsRefrn" value="${dataMapForM.pdsRefrn }" />
    ...
    <%@ include file="/WEB-INF/include/topbar.jsp" %>
           ...
           <div class="content">
              ...
              <form enctype="multipart/form-data" role="form" method="post" id="registForm">
                ...
                <input type="hidden" name="m_unityatchmnflno" value="" id="m_unityatchmnflno"/>
                    ...
                    <div class="archive-regite">
                        <div class="archive-info">
                            <table>
                                ...
                                <tbody>
                                    <tr>
                                    <c:if test="${not empty pdsFolderDeltail}">
                                     <c:forEach items="${pdsFolderDeltail }" var="folderM">
                                        <td>폴더명</td>
                                        <td colspan="3">
                                        
                                        	<input type="hidden" id="e_title" name="e_title">
                                        	<input type="hidden" id="d_title" name="d_title">
                                            <input type="text" id="m_title" name="m_title" value="${folderM.m_title }" autocomplete="off">
                                        </td>
                                      </c:forEach>
                                    </c:if>
                                    </tr>
                                    ...
                                    <tr>
                                        <td colspan="3">
                                            ...
                                            <c:if test="${not empty pdsRefrn[0].emp_nickname }">
                                            <c:forEach items="${pdsRefrn }" var="refrn">
                                            	<span class="add-m">${refrn.emp_nickname }<i class="fa-solid fa-xmark"></i>
                                            		<input type="hidden" value="${refrn.emp_nickname }" id="${refrn.emp_nickname }" name="ccEmpList">
                                            	</span>
                                            </c:forEach>
                                            </c:if>
                                            </div>
                        ...
                        <div class="archive-upload">
                            ...
                            <c:forEach items="${pdsAttachList }" var="attach">
	                            <div id="file0" class="reference-list fileArea mb-20">
		                            <div class="refer-con flex">
			                            <div class="refo refo01">
			                            	<i class="fa-solid fa-file-lines"></i>
			                            </div>
			                            <div class="refo refo02">
				                            <div class="refocon flex ju-sp-bt">
					                            <span class="fileN">${attach.originalname}</span>
					                            <button class="detBtn">CANCEL</button>
				                            </div>
				                            <div class="refocon roding-bar"></div>
				                            <div class="file-size">${attach.filesize}
				                            	<em>%78 upload</em>
				                            </div>
			                            </div>
		                            </div>
      ...
      //fileNo 속성 추가
      const fileAreas = document.querySelectorAll('.fileArea');
     for(let i =0; i<fileAreas.length; i++){
    	 fileAreas[i].setAttribute("id","file"+i);
     }
     
     //파일아이콘 버튼 속성 추가
     const detBtns = document.querySelectorAll('.detBtn');
     for(let i =0; i<detBtns.length; i++){
    	 detBtns[i].setAttribute("onclick","deleteFile("+i+");");
     }
    	...
    	// 첨부파일 개수 확인
          ...
          for (var i = 0; i < Math.min(curFileCnt, remainFileCnt); i++) {
             ...
             /*  if (validation(file)) { */
                  ...
                  let htmlData = '';
                  htmlData += '<div id="file' + fileNo + '" class="reference-list fileArea mb-20">';
