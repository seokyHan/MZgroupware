...
<%-- <c:set var="pdsDepList" value="${dataMap.pdsDepList }" /> --%>
        ...
        <div class="modal-con modal27">
            ...
            <div class="content">
                    <div class="txt-sear-box flex ju-sp-bt al-it-ce">
                        <h1>부서자료실 폴더<span>${loginEmp.dept }</span></h1> 
                    ...
                    <div class="archive">
                        ...
                        <ul class="fol-list flex flex-wrap">
                            <li class="add-fol">
							 ...
							 <c:forEach items="${pdsDepList }" var="pdsd">
                            <li class="folderList">
                                <div class="remove-check">
                                	<c:if test="${loginEmp.ncnm eq pdsd.d_writer }">
                                    	<input name="checkNo" value="${pdsd.d_unityatchmnflno}" type="checkbox">
                                    </c:if>
                                </div>
                                <img src="<%=request.getContextPath() %>/resources/images/aca.png" onclick="list_goo('pdsDepList','${pdsd.d_unityatchmnflno}','${pdsd.d_title}','${pdsd.d_writer }');">
                                <div class="arch-name">
                                    <span>${pdsd.d_title }
                                    	<input type="hidden" class="d_writer" value="${pdsd.d_writer}">
                                    	<input type="text" name="d_title" class="d_title toDisplay" autocomplete="off" style="display:none"/> 
                                    	<input type="hidden" class="d_unityatchmnflno" value="${pdsd.d_unityatchmnflno}">
                                    	<c:if test="${loginEmp.ncnm eq pdsd.d_writer }">
                                    		<i class="fa-solid fa-pen"></i>
                                    	</c:if>
                                    </span>
                                </div>
                                <div class="fol-date" onclick="list_goo('pdsDepList','${pdsd.d_unityatchmnflno}','${pdsd.d_title}','${pdsd.d_writer }');">
                                    <em>생성일</em> <fmt:formatDate value="${pdsd.d_regdate }" pattern="yyyy-MM-dd"/>
                                </div>
                            </li>
                            </c:forEach>
                        </ul>
 ...
 const folder_length = document.getElementsByName("checkNo").length;
	...
	for(let i=0; i<folder_length; i++){
		...
		if(document.getElementsByName("checkNo")[i].checked == true){
			d_unityatchmnflno.push(document.getElementsByName("checkNo")[i].value);
		}
	}
			if(d_unityatchmnflno.length == 0){
				alert("삭제할 폴더를 선택하세요");
			}
			else{
				
				$.ajax({
					url : '<%=request.getContextPath()%>/pds/countFilesForD',
					type : 'post',
					data : JSON.stringify(d_unityatchmnflno),
					...
					success : function(result){
						...
						deleteFolder(d_unityatchmnflno);
					},
...
function deleteFolder(d_unityatchmnflno){
		$.ajax({
			url : '<%=request.getContextPath()%>/pds/deleteFolderForD', 
			type : 'post', 
			data : JSON.stringify(d_unityatchmnflno), 
			contentType : 'application/json', 
			success : function(result){ 
...
modifyNow.addEventListener('click', function(){
	...
	const data = {
		d_title : name.value,
		d_unityatchmnflno : unityNo.value
	}
	
	console.log(data.d_unityatchmnflno);
	
 		$.ajax({
 			url : '<%=request.getContextPath()%>/pds/modifyFolderNameForD', 
 			type : 'post', 
...
function list_goo(url,d_unityatchmnflno,d_title,d_writer){	
	location.href=url+"?d_unityatchmnflno="+d_unityatchmnflno+"&d_title="+d_title+"&d_writer="+d_writer;
}

const unityatchmnflno = document.querySelectorAll('.d_unityatchmnflno');
